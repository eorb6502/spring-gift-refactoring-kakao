...
- 관련 파일:
  - `src/main/java/gift/product/ProductController.java` (REST API)
  - `src/main/java/gift/product/AdminProductController.java` (Thymeleaf 관리자 페이지)
  - `src/main/java/gift/product/Product.java` (엔티티)
  - `src/main/java/gift/product/ProductRepository.java`
  - `src/main/java/gift/product/ProductRequest.java`
  - `src/main/java/gift/product/ProductResponse.java`
  - `src/main/java/gift/common/NameValidator.java` (검증 유틸)
  - `src/main/java/gift/category/CategoryRepository.java` (카테고리 조회에 사용)
...
1. `ProductService` 클래스 생성 (`src/main/java/gift/product/ProductService.java`)
   - `@Service` 어노테이션
