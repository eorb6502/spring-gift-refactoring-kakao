...
분석 보고서:
- `docs/refactoring-analysis-category-service-extraction.md`
- `docs/refactoring-analysis-product-service-extraction.md`
...
| 파일 | 설명 |
|------|------|
| `gift/category/CategoryService.java` | 카테고리 CRUD 서비스. `findAll()`, `findById()`, `create()`, `update()`, `delete()` |
| `gift/product/ProductService.java` | 상품 CRUD 서비스. NameValidator 검증(`checkKakao=true`) 포함. CategoryService 의존. |
